...
async function afficherEnregistrements() {
  ...
  try {
    // Si aucune donnée n'est trouvée, on utilise un tableau vide
    ...
    // Parcourir les groupes et créer les éléments d'affichage
...
afficherPopupAideCustom(
  ...
  'pop_edit'
    ...
    // Conteneur complet avec bouton ✏️ + date modifiable + résumé modifiable
...
cancelButton.textContent = 'Annuler les mofifications';
cancelButton.className = 'button';
cancelButton.style.display = 'none';

// Fonction : activer les boutons
function showEditButtons() {
  saveButton.style.display = 'inline-block';
  cancelButton.style.display = 'inline-block';
}

// Fonction : restaurer état original
...
// Sauvegarde
saveBtnTemps.addEventListener('click', async () => {
  const [h, m] = inputTemps.value.split(':');
  const regex = /(?:(\d+)j)?\s*(?:(\d+)h)?\s*(?:(\d+)m)?\s*(?:(\d+)s)?/;
  const match = regex.exec(techEnregistrement.temps);

  const jours = match && match[1] ? `${match[1]}j ` : '';
  const secondes = match && match[4] ? `${match[4]}s` : '';

  const nouvelleValeurTemps = `${jours}${parseInt(h)}h ${parseInt(m)}m ${secondes}`.trim();

  techEnregistrement.temps = nouvelleValeurTemps;

  const groupe = enregistrements.filter(e => e.id === enregistrement.id && e.Tech === techEnregistrement.Tech);
  groupe.forEach(e => {
    e.temps = nouvelleValeurTemps;
  });
...
enregistrementDiv.appendChild(editButtonsTempsContainer);
          ...
          // Ajouter le bouton "Restaurer" seulement si le paramètre Export n'est pas présent
...
if (isNaN(maxDateObj.getTime())) {
    console.error("❌ Date invalide pour maxDate :", maxDate);
    alert("La date maximale (date_LM) est invalide ou manquante.");
    ...
    // Génération des options pour les types et causes
    const typesOptions = lieuData.types.map(type => `<option value="${type}"${enregistrement.zoneTexte4 === type ? ' selected' : ''}>${type}</option>`).join('');
    const causesOptions = lieuData.causes.map(cause => `<option value="${cause}"${enregistrement.zoneTexte5 === cause ? ' selected' : ''}>${cause}</option>`).join('');
    ...
    modalDiv.innerHTML = `
        <div class="modal-content">
            ...
            <div>
                <label for="typePanneSelect">Type de panne :</label>
                <select id="typePanneSelect">${typesOptions}</select>
            </div>
            <div>
                <label for="causePanneSelect">Cause de la panne :</label>
                <select id="causePanneSelect">${causesOptions}</select>
            </div>
            <div>
