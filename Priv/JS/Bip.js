...
function gestdata(scannedText) {
    if (scannedText.startsWith('scan-') && scannedText.endsWith('-fin')) {
        ...
        const processParams = (params) => {
            ...
            if (rawF) {
                const decodedF = nettoyerNomLieu(rawF);
                ouvrirIframe(decodedF);
            } else if (rawI) {
                const decodedI = nettoyerNomLieu(rawI);
                modalselect(decodedI, "Priv");
            } else if (rawP) {
                const decodedP = nettoyerNomLieu(rawP);
                modalselect(decodedP, "Priv");
            } else {
                afficherResult(`Le QR ne permet pas de créer une nouvelle intervention: ${scannedText}`);
            }
        };
        ...
        try {
            if (/^https?:\/\//i.test(content)) {
                ...
                // Cas 4 : contenu brut pur
                modalselect(content, "Priv");
            }
        } catch (error) {
...
function nettoyerNomLieu(input) {
    ...
    try {
        ...
        console.warn("Erreur de décodage pour :", input);
        return input;
    }
}
...
function displayUsernameAndLogout() {
 ...
 <p>
    ...
    if (interAc) {
        ...
        setTimeout(() => {
            modalselect(decodedparamP, "Prev");
